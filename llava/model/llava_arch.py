...
class LlavaMetaForCausalLM(ABC):
    ...
    def prepare_inputs_labels_for_multimodal_pdrop(
        ...
        # TODO: 实现
        self.model.image_token_posi = image_token_posi  # LlavaLlamaForCausalLM_PDrop.model(LlavaLlamaModel).
    ...
    # TODO：OurDrop实现
    def prepare_inputs_labels_for_multimodal_topk(
        self, input_ids, position_ids, attention_mask, past_key_values, labels,
        images, image_sizes=None
    ):
        vision_tower = self.get_vision_tower()
        ...
        for batch_idx, cur_input_ids in enumerate(input_ids):
            ...
            if image_index == []:
                ...
                image_token_posi.append(image_index[0])
            # record seq_token_len for further dropping
            seq_token_len.append(cur_input_ids.shape[0])
        ...
        self.model.image_tokens = [image_feature.shape[0] for image_feature in image_features]
        self.model.seq_token_len = seq_token_len

        # Truncate sequences to max length as image embeddings can make the sequence longer
        # tokenizer_model_max_length = getattr(self.config, 'tokenizer_model_max_length', None)
        tokenizer_model_max_length = getattr(self.config, 'tokenizer_model_max_length', 2048)   # like pdrop
        if tokenizer_model_max_length is not None:
            new_input_embeds = [x[:tokenizer_model_max_length] for x in new_input_embeds]
            new_labels = [x[:tokenizer_model_max_length] for x in new_labels]
