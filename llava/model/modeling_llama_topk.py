...
class LlamaTopkModel(LlamaPreTrainedModel):
    """
    ...
    def forward(
        ...
        """
            ...
            "image_token_keep_list": self.image_token_keep_list,
            "image_token_posi": self.image_token_posi, 一阶段训练的时候<img>是在开头的，二阶段训练的时候<img>在<sys>之后
            "image_tokens": self.image_tokens,
            "seq_token_len": self.seq_token_len,
        """
        ...
        if self._use_flash_attention_2:
            """ shape: (bs, seq_len)
                mask内部的值都是0/1 (0=padding, 1=valid token)
                如果attn_mask全部都是1,直接为None就行,省掉unpad步骤还能加速,因为kernel本身就带causal mask
            """
            # 2d mask is passed through the layers
            attention_mask = attention_mask if (attention_mask is not None and 0 in attention_mask) else None
        elif self._use_sdpa and not output_attentions:
            """ shape: (bs, 1, q_len, k_len), mask: 0允许attn, -inf禁止attn """
        ...
        for layer_idx, decoder_layer in enumerate(self.layers):
            ...
            # print(f"image_token_keep_list: {self.image_token_keep_list}")
            # print(f"image_token_posi: {self.image_token_posi}")
            # print(f"image_tokens: {self.image_tokens}")
            # print(f"seq_token_len: {self.seq_token_len}")
            # print("------------------------")
        ...
        #     labels=labels  # 添加labels到返回值
        # )
        else:
            # 使用标准的 BaseModelOutputWithPast，不包含 labels
            ...
            # 添加一个属性来传递修改后的标签
            base_output.updated_labels = labels
            return base_output

    # TODO: 实现
    def rank_drop(self, cur_num, rank_layer, features, position_ids, attention_mask, labels):
        # step1: 初始化与参数准备
        _labels = labels
        ...
        if getattr(self.config, 'tokenizer_padding_side', 'right') == "right":  # step2: 处理右侧填充得情况，这是常见的填充方式
            ...
            # k
            image_tokens = [self.image_token_keep_list[cur_num] for _ in self.image_tokens]
            keep_length = [self.image_token_keep_list[cur_num + 1] for _ in self.image_tokens]
...
class LlamaForCausalLM(LlamaPreTrainedModel):
    ...
    def __init__(self, config):
        super().__init__(config)
        self.model = LlamaTopkModel(config)
        self.vocab_size = config.vocab_size
