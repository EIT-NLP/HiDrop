...
class LlamaModel(LlamaPreTrainedModel):
        ...
        for layer_idx, decoder_layer in enumerate(self.layers):
            ...
            if rank_layer in self.layer_list:
                if hidden_states.shape[1] != 1:  # prefill stage or training
                    ...
                    # a = attention_mask
                    # 更新 attention mask: process attention_mask again after undating
                    if self._use_flash_attention_2:
                        # 2d mask is passed through the layers
                        attention_mask = attention_mask if ( attention_mask is not None and 0 in attention_mask) else None
                    elif self._use_sdpa and not output_attentions:
                    ...
                    stage = self.layer_list.index(rank_layer)  # determine current stage
                    # 计算当前阶段每个样本中应保留的图像token数量
                    #   self.image_tokens是每个样本中原始图像token的总数
                    #   self.image_token_ratio_list[stage]是当前阶段的保留比例
                    #   对批次中每个样本计算当前阶段保留的图像token数量
                    cur_visual_length = [int(cur_image_token * self.image_token_ratio_list[stage]) for cur_image_token
                                         in self.image_tokens]
                    # 计算下一阶段每个样本中应保留的图像token数量
                    # 使用stage+1获取下一个丢弃阶段的保留比例
                    ...
                    for idx, cur_position_ids in enumerate(position_ids):
                        ...
                        # 将当前位置ID减去这个差值，以补偿丢弃的token
                        # 这确保了在token被丢弃后，位置编码仍然连续，不会出现"跳跃"
    ...
    # TODO: 实现
    def pdrop_rank_drop(self, cur_num, rank_layer, features, position_ids, attention_mask, labels):
        # step1: 初始化与参数准备
        _labels = labels
        ...
        if getattr(self.config, 'tokenizer_padding_side', 'right') == "right":  # step2: 处理右侧填充得情况，这是常见的填充方式
            ...
            # step4: 提取有效特征
            # take valid features
            features = [cur_features[cur_attention_mask] for cur_features, cur_attention_mask in
                        zip(features, attention_mask)]
            labels = [cur_labels[cur_attention_mask] for cur_labels, cur_attention_mask in zip(labels, attention_mask)]
            attention_mask = [cur_attention_mask[cur_attention_mask] for cur_attention_mask, cur_attention_mask in
                              zip(attention_mask, attention_mask)]
            ...
            for i in range(batch_size):
                ...
                if self.training:  # Training: 找到答案部分前的指令token作为查询
                    # 找到所有有标签的token位置（即答案部分）
                    answer_index = torch.where(labels[i] != -100)[0].tolist()   # ？？？
                    # print(f"答案部分: {answer_index}")
                    # 找到答案之前的指令token位置
                    index_before_answer = []
                    ...
                    # print(f"答案之前部分：{index_before_answer}")
                ...
                else:  # Inference: 使用提示部分最后一个token作为查询
                ...
                # print(f"111111111111111111111 {attention_avg_text.shape}")
                # print(f"222222222222222222222 {keep_length[i]}")
