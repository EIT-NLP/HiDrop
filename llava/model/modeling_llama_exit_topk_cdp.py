...
class LlamaExitTopkCdpModel(LlamaPreTrainedModel):
    """
    ...
    def forward(
        ...
        # print("\n---------------------------------------------")
        # if position_ids is None:
        #     print(f"original position_ids: {position_ids.shape}")
        # if attention_mask is None:
        #     print(f"original attention_mask: {attention_mask.shape}") # 可能是None
        # if hidden_states is None:
        #     print(f"original hidden_states: {hidden_states.shape}")
        # if labels is None:
        #     print(f"original label: {labels.shape}")
        ...
        for layer_idx, decoder_layer in enumerate(self.layers):
            ...
            if self.gradient_checkpointing and self.training:   # Training
                if layer_idx == 0:    # Before Late Entry: position id中预留了img token的位置
                    (
                        position_ids,
                        attention_mask,
                        hidden_states,
                ...
                elif layer_idx == self.late_entry_layer-1:    # Late Entry: 内部重新生成完整的连续position id就行，img token已经进来了
                    (
                        position_ids,
                        attention_mask,
                        hidden_states,
                ...
                #     print(f"Current {layer_idx} : total_seq_token_len = {total_seq_token_len}")
                # print(f"Current {layer_idx}")
                # print(f"noim: {noim_position_ids.shape} {noim_attention_mask.shape} {noim_labels.shape}")
            ...
            if rank_layer in self.layer_list:
                if hidden_states.shape[1] != 1:  # prefill stage or training
                    ...
                    # print(f">>>>>>>> topk: [{layer_idx}] >>>>>>>>")
                    # print(f"!!! topk: [{layer_idx}] {position_ids.shape} {attention_mask.shape} {hidden_states.shape} {labels.shape}")
                    ...
                    # print(f"---> [{layer_idx}] {position_ids.shape} {attention_mask.shape} {hidden_states.shape} {labels.shape}")
                # else:  # Decode stage：如果是CDP,不需要调整位置编码
    ...
    def rank_drop(self, cur_num, rank_layer, features, position_ids, attention_mask, labels, total_seq_token_len):
        ...
        if getattr(self.config, 'tokenizer_padding_side', 'right') == "right":  # step2: 处理右侧填充得情况，这是常见的填充方式
            ...
            for i in range(batch_size):
                ...
                temp_position_ids =  torch.arange(0, image_index+len(top_rank_index), dtype=position_ids.dtype, device=position_ids.device) # 为sys&img重新赋连续的Position ids
                new_position_ids = torch.cat([temp_position_ids, position_ids[i][start_index:]], dim=0)
...
class LlamaForCausalLM(LlamaPreTrainedModel):
    ...
    def __init__(self, config):
        super().__init__(config)
        self.model = LlamaExitTopkCdpModel(config)
        self.vocab_size = config.vocab_size
